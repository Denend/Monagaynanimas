  ...
  try {
    ...
    if (balance < estimatedCost) {
      if (score > 2 && lastSuccessfulTx && lastSuccessfulTx.address === userAddress && !scoreSubmitted && lastSuccessfulTx.hash !== lastSubmittedHash) {
        await setScoreOnDeath(lastSuccessfulTx.address, lastSuccessfulTx.score, lastSuccessfulTx.hash, user, accessToken, identityToken, gameStat);
        scoreSubmitted = true;
    ...
    if (error instanceof Error) {
      ...
      if (retryCount < 2 && (
        ...
        await new Promise(resolve => setTimeout(resolve, 2000));
        return sendTransaction(score, wallets, accessToken, identityToken, authenticated, isDead, retryCount + 1, user);
      }
...
export const setScoreOnDeath = async (address: string, score: number, hash_tx: string, user: any, accessToken: string | null, identityToken: string | null, gameStat: any) => {
  ...
  const setScoreOnDeath = await fetch("https://api.monfunimals.xyz/api/v1/game/dead", {
    method: "POST",
    headers: {
...
export const sendTransactionAsGuest = async ({score, isDead = false, unitType}: {score: number, isDead: boolean, unitType?: 'FLY' | 'FIRE_MOLANDAK'}) => {
  try {
    ...
    const firstResp = await fetch("https://api.monfunimals.xyz/api/v1/game/guest_mode/killed", {
      method: "POST",
...
export const sendTransactionAsGuest1 = async ({score, isDead = false, username} : {score: number, isDead: boolean, username?: string}) => {
  try {
...
export const getLeaderBoard = async () => {
  const response = await fetch("https://api.monfunimals.xyz/api/v1/guests/leaderboard",
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json"
...
export const getAuthLeaderBoard = async () => {
  const response = await fetch("https://api.monfunimals.xyz/api/v1/users/leaderboard",
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json"
...
export const checkMyScore = async (identityToken: string | null): Promise<any> => {
  const response = await fetch(`https://api.monfunimals.xyz/api/v1/users/me`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      "Privy-Id-Token": identityToken ? `Bearer ${identityToken}` : "",
...
export const faucet = async (address: string, identityToken: string | null): Promise<LeaderboardResponse> => {
  try {
    const response = await fetch(`https://api.monfunimals.xyz/api/v1/game/crane?address=${address}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Privy-Id-Token": identityToken ? `Bearer ${identityToken}` : "",
